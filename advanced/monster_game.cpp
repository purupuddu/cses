...
template<class T>
ostream& operator << (ostream &out, const deque<T>& vec){ out << "["; rep(i, 0, ssize(vec)) out << ' ' << vec[i]; out << " ]\n"; return out; }
...
double inters(Line a, Line b)
{
	return (double(b[0]-a[0]))/double(a[1]-b[1]);
}

int line_at(Line l, int x)
{
	return l[0]+x*l[1];
}
signed main()
{
	init();
	int n, sk; cin >> n >> sk;
	vi str(n), fac(n); cin >> str >> fac;
	deque<Line> q;
	q.push_back({0, sk});
	rep(i, 0, n)
	{
		while(q.size() >= 2 && line_at(q[0], str[i]) > line_at(q[1], str[i]))
			q.pop_front();
		Line l = {line_at(q[0], str[i]), fac[i]};

		while(q.size() >= 2 && q.back()[1] != l[1] && inters(l, q.back()) < inters(l, q[q.size()-2]))
			q.pop_back();
		if(q.back()[1] != l[1] || i == n-1)
			q.push_back(l);
		// cout<<q;
	}
	cout<<q.back()[0]<<endl;
