...
// move i disks from src to dest
void solve(int i, int src, int dest)
{
	if(i == 0)
		return;
	int other = 1;
	while(other == src || other == dest)
		other++;
	// move all disks above the ith on other
	solve(i-1, src, other);
	// now we can move the ith on dest
	cout<<src<<' '<<dest<<endl;
	// now we have to move all the disks we put on other onto dest
	solve(i-1, other, dest);
}
