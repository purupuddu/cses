...
struct SegmentTree {
	...
	Node unit = Node(0, 0); // s.t. for every x merge(x, unit) = x
	Node merge(Node a, Node b) { return {a.F + b.F, max(a.F+b.F, max(a.S, a.F+b.S)) }; }
	...
	SegmentTree(const vector<int>& data) {
		...
		st.assign(2*n, unit);
		for(int i = 0; i < ssize(data); i++) st[i+n] = Node{data[i], max(data[i], 0)};
		for(int i = n-1; i > 0; i--) st[i] = merge(st[2*i], st[2*i+1]);
	}
	void update(int p, int v) {
		st[p += n] = Node{v, max(v, 0)};
