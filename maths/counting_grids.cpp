...
ll bin_expo(ll n, ll pow, int mod)
{
	n %= mod;
	pow %= mod-1;
	ll res = 1;
...
signed main()
{
	init();
	int n; cin >> n;
	ll tot = bin_expo(2, (ll(n)*n)%(mod-1), mod);
	ll last = 0;
	ll cells = 0;
	rep(i, n&1, n+1)
	{
	...
	tot += (bin_expo(2, cells, mod)*3) % mod;
	ll cells2 = cells - (n&1);
	// cout<<cells<<' '<<cells2<<endl;
	tot += (bin_expo(2, cells, mod) * (bin_expo(2, cells2, mod) - 1 + mod)%mod)%mod;
	tot %= mod;
	// cout<<tot<<endl;
	tot *= 250000002;
	tot %= mod;
...
/*
tile the grid with this type of shape. 
Replicating the ### part 4 times gives
a rotation invariant grid. 
Also replicating both # and @ gives rotation invariant grids (only 2)
###.
.#..
..@.
.@@@
