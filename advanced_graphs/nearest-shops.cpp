...
signed main()
{
	...
	vi dist(n, 1e9), label(n, -1);
	vi special_dist(n, 1e9);
	queue<int> q;
	rep(i, 0, n)
	{
		if(is_special[i])
	...
	while(!q.empty())
	{
		...
		for(int x:gr[p])
		{
			if(label[x] != -1 && label[x] != l)
			{
				special_dist[l] = min(special_dist[l], d + dist[x] + 1);
				special_dist[label[x]] = min(special_dist[label[x]], d + dist[x] + 1);
			}
			if (dist[x] > d+1)
			{
				dist[x] = d+1;
